<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="ISO-8859-1">

<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha3/dist/css/bootstrap.min.css">

<title>User login</title>
<link rel="Website Icon" type="png" href="data:image/jpeg;base64," >

<link rel="stylesheet" href="css/userlogin.css">
<link rel="stylesheet" href="css/user.css">
<link rel="stylesheet" href="css/footer.css">
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css" integrity="sha512-iecdLmaskl7CVkqkXNQ/ZH/XLlvWZOJyj7Yy7tcenmpD1ypASozpmT/E0iPtmFIB46ZmdtAc9eNBvH0H/ZpiBw==" crossorigin="anonymous" referrerpolicy="no-referrer" />

</head>
<body>
<div class="text-center fs-3 fw-bold ">
  <p>Become a Member
It's free to join!</p>
  </div >
<form method="get" action="Userlogin" class=" text-center  fs-5 form-control container forrm shadow">
  <p class="fs-1 fw-bold mt-5  text-white ps-5 ms-5">Enter user information</p>
   <input type="text" placeholder="  first name" name="fname" class="  fw-bold mt-4 px-5 py-2 shadow mx-3 border rounded-3 fw-bold btn fname">
    
     <input type="text" placeholder="  last name" name="lname" class="mt-4 px-5 py-2 border rounded-3 shadow fw-bold btn"><br>
      <input type="number" placeholder=" mobile no" name="number" class="mt-4 px-5 py-2 border rounded-3 mx-3 shadow fw-bold btn">
       <input type="email" placeholder=" email" name="email" class="mt-4 px-5 py-2 border rounded-3 fw-bold shadow btn"><br>
       <input type="text" placeholder=" your address" name="address" class="mt-4 px-5 py-2 border rounded-3 shadow fw-bold btn"><br>
       <input type="text" placeholder="create password" name="pass" class="mt-4 px-5 py-2 border rounded-3 shadow mx-3 fw-bold btn">
       <input type="text" placeholder="re-enter password" name="repass" class="mt-4 px-5 py-2 border rounded-3 shadow fw-bold btn"><br>
       
     <button type="submit" class=" mt-4 px-4 py-1  fw-bold fs-5 mb-5  shadow rounded-3 text-white btn" >Sign In</button>
     <p>
     </p>
</form>
<section class="footerr  text-white text-center px-5 mx-4 py-3 mt-3 shadow">
    <div>
          <div>
           <p class="fs-3 fw-bold text-warning">Hotel Ashish Continental Pune</p>
           <p>Pune-Banglore Road, Near Balewadi Sports Complex, Balewadi, Pune 411045</p>
          </div>
          <ul class="list list-unstyled d-flex justify-content-around pt-3">
          <li><a href="" class="text-white"><i class="fa-regular fa-envelope"></i> resv.pune@hotelashishcontinental.com</a> </li>
          <li><a href="" class="text-white"><i class="fa-regular fa-envelope"></i> resv.pune@hotelashishcontinental.com</a> </li>
          <li><a href="" class="text-white"><i class="fa-solid fa-phone">+91 91 6916 6789 | Central Reservation</i></a></li>
         <li> <a href="" class="text-white"><i class="fa-solid fa-phone"></i> +91 20 6791 4040 | Hotel</a></li>
          </ul> 
          <div>
            <p class="fs-5 fw-bold pt-3">Get In Touch</p>
            <ul class="list1 list-unstyled d-flex pb-3">
            <li class="list2 mx-3"><a href="" class="text-white face px-4 py-2 rounded-3 bg-secondary fs-5"><i class="fa-brands fa-facebook"></i></a></li>
            <li class="list2 mx-3"><a href="" class="text-white px-4 py-2 rounded-3 bg-secondary fs-5"><i class="fa-brands fa-twitter"></i></a></li>
            <li class="list2 mx-3"><a href="" class="text-white px-4 py-2 rounded-3 bg-secondary fs-5"><i class="fa-brands fa-instagram"></i></a></li>
            <li class="list2 mx-3"><a href="" class="text-white px-4 py-2 rounded-3 bg-secondary fs-5"><i class="fa-brands fa-youtube"></i></a></li>
            </ul>
          </div>
          <hr>
          <div>
          <p class=" fs-5">&copy;All rights reserved to Hotel Ashish Continental,Pune.</p>
          </div>
          
    </div>
    </section>
</body>
</html>